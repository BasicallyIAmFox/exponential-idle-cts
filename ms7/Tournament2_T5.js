...
var id = "logistic_function_ms7"
var name = "Logistic Function (Mid-Season 7)";
var description = "An implementation of the 'Logistic Function' theory from the game. You also have 30 minute limit to go as far as possible starting from 0 ρ and 0R9! Good luck!";
...
var getInternalState = () => `${q} ${timerStarted} ${timer.toString()} ${cumul_rho.toString(4)}`

var setInternalState = (state) => {
    let values = state.split(" ");
    if (values.length > 0) q = parseBigNumber(values[0]);
    if (values.length > 1) timerStarted = values[1].toLowerCase() === 'true';
    if (values.length > 2) timer = parseFloat(values[2]);
    if (values.length > 3) cumul_rho = parseBigNumber(values[3]);
    saveScum = true;
}
