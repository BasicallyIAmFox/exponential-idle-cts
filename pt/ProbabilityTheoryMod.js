...
var tick = (elapsedTime, multiplier) => {
    ...
    if (theory.isAutoBuyerActive) {
        ...
        if (autobuyCooldown <= 0) {
            let extra_upgrades = [];
            if (stage !== 0) {
                extra_upgrades = [...extra_upgrades, b1, c1, c2];
                if (cTerms.level > 0) extra_upgrades.push(c3);
                if (cTerms.level > 1) extra_upgrades.push(c4);
            }
            if (stage !== 1 && cur2Unlock.level > 0) {
                extra_upgrades = [...extra_upgrades, b2, n1, n2];
                if (skills[11].level > 0) extra_upgrades.push(c5);
                if (skills[11].level > 1) extra_upgrades.push(c6);
                if (skills[11].level > 2) extra_upgrades.push(c7);
                if (skills[11].level > 3) extra_upgrades.push(c8);
            }
            if (stage !== 3 && cur3Unlock.level > 0) {
                extra_upgrades = [...extra_upgrades, b3, n3, n4];
                if (skills[20].level > 0) extra_upgrades.push(c9);
                if (skills[20].level > 1) extra_upgrades.push(c10);
                if (skills[20].level > 2) extra_upgrades.push(c11);
                if (skills[20].level > 3) extra_upgrades.push(c12);
            }
            if (stage !== 4 && diceUnlock.level > 0) {
                extra_upgrades = [...extra_upgrades, d4, d6, d8, d10, d12, d20];
            }
...
var randSum = (itr) => {
    if (itr < 100) {
        ...
        let result = gaussianRandom(itr / 2, (itr / 12).sqrt());

        return BigNumber.from(result); // safety conversion
    }
};

var gaussianRandom = (mean, stdev) => {
